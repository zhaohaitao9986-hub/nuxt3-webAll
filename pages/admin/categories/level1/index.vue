...
const canDelete = computed(() => userStore.user?.role === 'SUPERADMIN')
...
async function loadList() {
  ...
  try {
    ...
    const res = await useAdminFetch('/api/admin/categories/level1', { query })
    list.value = res.data || []
...
async function saveDialog() {
  ...
  try {
    if (editingId.value == null) {
      await useAdminFetch('/api/admin/categories/level1', {
        method: 'POST',
        body: {
          name: form.name.trim(),
      ...
      await useAdminFetch(`/api/admin/categories/level1/${editingId.value}`, {
        method: 'PUT',
        body: {
          name: form.name.trim(),
          handle: form.handle.trim(),
...
async function onDelete(row) {
  try {
    await ElMessageBox.confirm(
      `删除一级分类「${row.name}」将级联删除其下所有二级分类及工具关联，确定吗？`,
      '删除确认',
  ...
  try {
    await useAdminFetch(`/api/admin/categories/level1/${row.id}`, { method: 'DELETE' })
...
<template>
  <div class="l1-page">
    <el-card shadow="never" class="l1-filter-card">
    ...
    <el-card shadow="never" class="l1-table-card">
    ...
    <el-dialog
      ...
      :title="editingId == null ? '新建一级分类' : '编辑一级分类'"
      width="480px"
      destroy-on-close
...
.l1-filter-card :deep(.el-card__body) {
  padding-bottom: 4px;
}

.l1-table-card :deep(.el-card__body) {
  padding-top: 12px;
}
