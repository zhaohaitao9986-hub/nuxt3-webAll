...
async function crawlCategoryDetail(categoryHandle) {
  ...
  while (hasNextPage) {
    ...
    //   await saveToolsToDB(toolsData, category.id);
      ...
      if (currentPage > 3) { 
        hasNextPage = false; 
        console.log("⚠️ 测试模式：仅爬取前3页，如需全量请修改代码中的终止条件");
      } 
...
// --- 启动 ---
// 你可以从数据库读取所有需要爬取的 handle 循环，这里先演示一个
const TARGET_CATEGORY_HANDLE = 'ai-blog-generator'; 

crawlCategoryDetail(TARGET_CATEGORY_HANDLE)
  .catch((e) => {
    console.error(e);
    process.exit(1);
  })
