...
const START_FROM_HANDLE = 'dex-by-thirdlayer';
...
async function scrapeAndUpdateTool(handle, workerId) {
    ...
    await page.setDefaultTimeout(10000); // 全局默认等待10秒
...
async function batchScrapeAll() {
    console.log(`🚀 启动多并发爬取 | 并发数: ${CONCURRENCY} | 超时自动跳过`);
    ...
    async function worker(workerId) {
        console.log(`[W${workerId}] 启动`);
        while (taskQueue.length > 0) {
            const tool = taskQueue.shift();
            if (!tool) break;

            // 这里也加一层捕获，保证绝对不崩
            const res = await scrapeAndUpdateTool(tool.handle, workerId).catch(() => ({ success: false }));
