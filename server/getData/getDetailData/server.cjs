    ...
    try {
      ...
      const aiTool = await prisma.aiTool.upsert({
        ...
        update: {
          ...
          collectedCount: item.collected_count || 0,
          description: item.description,
          image: item.image,
          monthVisitedCount: BigInt(item.month_visited_count || 0),
          name: item.name,
          whatIsSummary: item.what_is_summary,
          isAd: item.is_ad || false,
          websiteName: item.website_name,
          isFree: item.is_free || false,
          websiteLogo: item.website_logo,
        },
        create: {
          website: item.website,
          collectedCount: item.collected_count || 0,
